...
def parse_gene_metadata(gff_file: str) -> pd.DataFrame:
    """Parse GFF to extract gene_biotype and NCBI GeneID for each gene feature."""
    records = []
        ...
        for line in f:
            ...
            for attr in attributes.split(';'):
                ...
                if attr.startswith('ID='):
                    gff_id = attr.replace('ID=', '')
                elif attr.startswith('gene_biotype='):
                    gene_biotype = attr.replace('gene_biotype=', '')
                elif attr.startswith('Dbxref='):
                    for ref in attr.replace('Dbxref=', '').split(','):
                        if ref.startswith('GeneID:'):
                            ncbi_gene_id = ref.replace('GeneID:', '')
                            break
            if gff_id is not None:
                records.append({
                    'gene_id': gff_id,
...
def main():
    ...
    # Filter to protein-coding genes and add NCBI GeneID
    gene_meta = parse_gene_metadata(args.gff_file)
    gene_windows_df = gene_windows_df.merge(gene_meta, on='gene_id', how='left')
    n_before = len(gene_windows_df)
    gene_windows_df = gene_windows_df[gene_windows_df['gene_biotype'] == 'protein_coding'].copy()
    print(f"Filtered to {len(gene_windows_df)} protein-coding genes (excluded {n_before - len(gene_windows_df)} tRNA/rRNA/other)")
    ...
    if not per_gene_df.empty:
        id_map = gene_windows_df.set_index('gene_id')['ncbi_gene_id']
        per_gene_df['gene_id'] = per_gene_df['gene_id'].map(id_map).fillna(per_gene_df['gene_id'])
